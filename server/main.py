...
# In-memory storage for restocking orders and tasks
restocking_orders = []
tasks = []
...
def submit_restocking_order(request: RestockingOrderRequest):
    ...
    order_number = f"RST-{datetime.now().strftime('%Y%m%d')}-{len(restocking_orders) + 1:04d}"

    # Calculate dates
    order_date = datetime.now().strftime('%Y-%m-%d')
    expected_delivery_date = (datetime.now() + timedelta(days=request.estimated_delivery_days)).strftime('%Y-%m-%d')
