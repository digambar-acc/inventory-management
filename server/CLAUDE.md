...
- Complex filtering or data transformation logic
- Workarounds for Pydantic validation quirks
...
**Function docstrings:**
```python
# ✅ Good - clear docstring with purpose and edge cases
def filter_orders_by_month(orders: List[Order], month: str) -> List[Order]:
    """
    Filter orders by month, supporting both YYYY-MM format and quarters.

    Args:
        orders: List of order objects to filter
        month: Month string in format 'YYYY-MM' or 'Q1-YYYY'

    Returns:
        Filtered list of orders matching the month/quarter

    Note:
        Returns empty list if month format is invalid rather than raising exception
    """
    # Implementation...
```
...
# Check if warehouse is not 'all'
if warehouse and warehouse != 'all':
    filtered = [item for item in filtered if item['warehouse'] == warehouse]

# ✅ Good - explains why this check matters
# Skip warehouse filter if 'all' selected to match frontend filter behavior
# where 'all' means "show everything from all warehouses"
if warehouse and warehouse != 'all':
    filtered = [item for item in filtered if item['warehouse'] == warehouse]
```

**Document complex list comprehensions:**
```python
# ✅ Good - break down complex logic
# Calculate total spending by category, excluding cancelled orders
# and grouping by lowercase category name for case-insensitive matching
category_totals = {
    cat.lower(): sum(order['total'] for order in orders
                     if order['status'] != 'cancelled'
                     and order['category'].lower() == cat.lower())
    for cat in unique_categories
}
```

**When NOT to Comment:**
- Self-explanatory variable names and function calls
- Standard CRUD operations
- Simple Pydantic model definitions
- Obvious filter checks
