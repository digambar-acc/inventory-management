...
- Complex computed property calculations
- Workarounds for Vue reactivity limitations
- Non-standard component communication patterns
- Performance optimizations (debouncing, lazy loading)
- Edge cases in data transformations
- Interactions between filters and API calls

**Vue/JavaScript Documentation Best Practices:**

**JSDoc for functions:**
```javascript
// ✅ Good - clear JSDoc with parameters and return type
/**
 * Calculate filtered inventory items based on selected filters.
 * Note: Does not apply month filter as inventory has no time dimension.
 *
 * @param {Array} items - Raw inventory items from API
 * @param {Object} filters - Current filter selections
 * @returns {Array} Filtered inventory items
 */
const getFilteredInventory = (items, filters) => {
  // Implementation...
}
```

**Comment complex computed properties:**
```javascript
// ✅ Good - explain the business logic
const revenueProgress = computed(() => {
  // Calculate progress toward $800K monthly goal
  // Using single month data, not YTD (which would be $9.6M)
  const monthlyGoal = 800000
  const actualRevenue = currentMonthRevenue.value
  return (actualRevenue / monthlyGoal) * 100
})
```

**Document Vue reactivity patterns:**
```vue
<script>
// ✅ Good - explain why .value is needed here
const updateFilters = () => {
  // Must use .value because searchQuery is a ref
  // Auto-unwrapping only works in templates
...
**Explain non-obvious v-for keys:**
```vue
<!-- ✅ Good - comment explains key choice -->
<!-- Using 'sku' as key instead of 'id' because items may share IDs
     across warehouses but SKU is globally unique -->
<div v-for="item in inventory" :key="item.sku">
```

**Comment workarounds:**
```javascript
// ✅ Good - documents why unusual approach is needed
// Wrap in nextTick to ensure DOM updates before recalculating chart dimensions
// This fixes a race condition where chartData updates before container resize
await nextTick()
recalculateChartBounds()
```

**When NOT to Comment:**
- Simple ref declarations (`const count = ref(0)`)
- Standard template syntax (`v-if`, `v-for` with obvious logic)
- Self-explanatory computed properties
- Straightforward API calls
- Basic prop definitions

**Template Comments:**
```vue
<!-- Use HTML comments in templates for component-level notes -->
<template>
  <div class="dashboard">
    <!-- Filter section intentionally placed above charts for mobile UX -->
    <FilterPanel />
